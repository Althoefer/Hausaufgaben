...
public class MyArrayList {
	...
	public void add(int i) {
		...
		if (this.size == this.arr.length) {
			int[] help = this.arr.clone();
			this.arr = new int[help.length * 2];
			// arr[] wird in ein doppelt so grosses Array kopiert
			System.arraycopy(help, 0, this.arr, 0, help.length);
		}
	...
	/**
	 * gibt das Element an der Stelle pos zurueck wenn pos nicht in der Liste
	 * steht wird eine Exception geworfen
	 * 
	 * @param pos
	 *            Position des gewuenschten Elements
	 * @return int gewuenschtes Element, falls vorhanden
	...
	/**
	 * gibt dies Liste als String zurueck in der Form [1,2,3,4,...,n]
	 * 
	 ...
	 * 
	 * @return int Positon des naechsten leeren Elements
	 */
	public int size() {
		return this.size;
	}
	 ...
	 * beachtet!
	 * 
	 * @return MyArrayList Klon des this-Objekts
	 */
	@Override
	...
	/**
	 * fuegt das Element i an der Stelle pos ein wenn pos keine gueltige
	 * Position ist, wird eine Exception geworgen
	 * 
	 * @param i
	...
	public void add(int i, int pos) throws ArrayIndexOutOfBoundsException {
		...
		if ((pos >= 0) && (pos < this.size)) {
			...
			// kopiert
			System.arraycopy(this.arr, pos, this.arr, pos + 1, this.arr.length - pos - 1);
	 ...
	 * eine Position nach vorne wenn arr[] dannach dreimal so groß ist wie size,
	...
	public void delete(int pos) throws ArrayIndexOutOfBoundsException {
		...
		if ((pos >= 0) && (pos < this.size)) {
			...
			if ((this.size - 1) <= (this.arr.length / 3)) {
				// Array wird in ein halb so grosses Array kopiert
				int[] help = this.arr.clone();
				this.arr = new int[help.length / 2];
				System.arraycopy(help, 0, this.arr, 0, this.size);
			}
			// kopiert alle Elemente hinter pos einen nach vorne
			// pos "verschwindet" dadurch
			System.arraycopy(this.arr, pos + 1, this.arr, pos, this.arr.length - pos - 1);
			this.size--;
		} else {
			throw new ArrayIndexOutOfBoundsException("Position nicht vorhanden!");
		}
