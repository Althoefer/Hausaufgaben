...
public class Rechenoperationsliste {
	...
	/**
	 * default constructor
	 * initialises the List of operations
	 */
	...
	/**
	 * transforms a field of double
	 * uses the list of operations and executes them in order on every element in the field
	 * @param feld		the field to use the operations on
	...
	public double[] transform(double[] feld){
		...
		for(int i = 0; i<temp.length; i++){
			for(int j = 0; j<operations.size(); j++){
				Rechenoperation current = operations.get(j);
