...
public class VectorRn {
	...
	public VectorRn add(VectorRn v2) throws IllegalArgumentException {
		...
		double[] summe = new double[this.vector.length];
		// jede Stelle wird addiert in einem Hilfsarray summe[] gespeichert
		for (int i = 0; i < this.vector.length; i++) {
			summe[i] = this.vector[i] + vector2[i];
		}
		return new VectorRn(summe);
	}

	/**
	 * gibt als Ergebnis die Multiplikation des this-Objekts mit dem Faktor d
	 * zurueck das this-Objekt wird dabei nicht veraendert
	 */
	public VectorRn mult(double d) {
		double[] mult = new double[this.vector.length];
		for (int i = 0; i < this.vector.length; i++) {
			mult[i] = this.vector[i] * d;
		}
		return new VectorRn(mult);
	}

	/**
	 * statische Methode berechnet das Skalarprodukt von v1 und v2
	 */
	...
	public boolean isParallel(VectorRn v2) throws IllegalArgumentException {
		...
		for (int i = 1; i < this.vector.length; i++) {
			// wenn der Faktor fuer die Elemente an der i-ten Stelle dem Faktor
			// an Stelle 0 1 entspricht
	...
	public VectorRn[] bestimmeSenkrechteVektoren() {
		...
		aussen: for (i = 0; i < this.vector.length; i++) {
			for (int j = +1; j < this.vector.length; j++) {
				...
				// count = 0 heisst senkrecht ist voll und es sind genuegend
				// senkrechte bestimmt
				if (count == 0) {
					break aussen;
				}
	...
	public double[] getVector() {
		double[] vectorR = this.vector.clone();
		return vectorR;
	}
	...
	public static void main(String[] args) {
		try {
			/*
			 * Fehler wenn nur ein Element des Vektors ungleich 0 ist, z.B.
			 * (0,0,0,1)
			 */
